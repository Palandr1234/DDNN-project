...
def Patch2Im(Y, win, img_size,stride=1):
    ...
    img = img/(weight+1e-6)
    return img
def prepare_data(data_path,sigma, patch_size, stride, aug_times=2):
    # train
        ...
        for k in range(len(scales)):
            ...
            for n in range(patches.shape[3]):
                ...
                for m in range(aug_times-1):
                    data_aug = data_augmentation(data, np.random.randint(8))

                    data_noisy_aug = data_augmentation(data_noisy, np.random.randint(8))
